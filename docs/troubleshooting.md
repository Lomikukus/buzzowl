...
| The `camofox` build fails on `curl … camoufox-152.0.4-beta.28-lin.<arch>.zip` | the build arch does not match the host | x86_64 hosts need **both** `CAMOFOX_ARCH=x86_64` and `CAMOFOX_BUILD_ARCH=x86_64` in `.env`, then `docker compose build camofox` |
...
## Agents get 403/503 or "(no readable content)" on most corporate sites

Check that the anti-detection browser really runs:

```bash
curl -s http://127.0.0.1:9377/health
```

`"browserRunning": true` is the only healthy answer. `false` with the container
reported healthy means the Camoufox image was built from a truncated archive
(the upstream Dockerfile ignores unzip errors, so `libxul.so` and
`properties.json` can be missing while the HTTP wrapper still answers). The
container log then repeats `camoufox launch attempt failed`. Fix:

```bash
docker compose build --no-cache camofox && docker compose up -d --force-recreate camofox
```

`scripts/build-browser.sh` refuses such an image. The compose healthcheck
requires `browserRunning` since 2026-09, so an unhealthy camofox after a
two-minute start period points at exactly this.
